...
async function deriveAllBlockchainAddresses(wif) {
  ...
  try {
    if (typeof floEthereum !== 'undefined' && typeof bitjs !== 'undefined') {
        const decoded = bitjs.wif2privkey(wif);
        if (decoded && decoded.privkey) {
            const rawPriv = decoded.privkey.substring(0, 64);
            const ethAddress = floEthereum.ethAddressFromPrivateKey(rawPriv);
            addresses.eth = ethAddress;
            addresses.bsc = ethAddress;
            addresses.matic = ethAddress;
            addresses.hbar = ethAddress;
            addresses.arb = ethAddress;
            addresses.op = ethAddress;
            addresses.avax = ethAddress;
        }
    }
  } catch (e) {
    console.warn("ETH/BSC/MATIC/HBAR/ARB/OP derivation failed:", e);
  }
  try {
